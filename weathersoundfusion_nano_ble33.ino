...
int posWindow = W_OPEN;
int posLouver = L_OPEN;
int selected;
...
void loop() {
    ...
    //RAIN DETECTED
    if (rainCount >= 4) {
      ei_printf("\nSELECTED: Rain\r\n");       // window close, louver open
      rainCount = 0;
      //CLOSE THE WINDOW WERVO
      if (posWindow >= W_OPEN) {   // from W_OPEN140 to W_CLOSE0
        for (posWindow = W_OPEN; posWindow >= W_CLOSE; posWindow--) {
          window.write(posWindow);
          delay(5);
        }
      }
      else if (posWindow == W_MID || posWindow == W_MID+1 || posWindow == W_MID-1) {   // from W_MID60 to W_CLOSE0
        for (posWindow = W_MID; posWindow >= W_CLOSE; posWindow--) {
          window.write(posWindow);
          delay(5);
        }
      }
      else {
        posWindow = W_CLOSE;    //keep W_CLOSE
      }
      //OPEN THE LOUVER SERVO
      if (posLouver <= L_CLOSE) {   // from L_CLOSE80 to L_OPEN160
        for (posLouver = L_CLOSE; posLouver <= L_OPEN; posLouver++) {
          louver.write(posLouver);
          delay(5);
        }
      }
      else if (posLouver == L_MID || posLouver == L_MID+1 || posLouver == L_MID-1) {   // from L_MID120 to L_OPEN160
        for (posLouver = L_MID; posLouver <= L_OPEN; posLouver++) {
          louver.write(posLouver);
          delay(5);
        }
      }
      else {
        posLouver = L_OPEN;   //keep L_OPEN
      }
    }
    ...
    //THUNDER DETECTED
      ...
      if (posWindow >= W_OPEN) {   // from W_OPEN140 to W_CLOSE0
        ...
        posWindow = W_CLOSE;    //keep W_CLOSE0
      }
      //CLOSE THE LOUVER SERVO
      if (posLouver >= L_OPEN) {   // from L_OPEN160 to L_CLOSE80
        for (posLouver = L_OPEN; posLouver >= L_CLOSE; posLouver--) {
          louver.write(posLouver);
          delay(5);
        }
      }
      else if (posLouver == L_MID || posLouver == L_MID+1 || posLouver == L_MID-1) {   // from L_MID120 to L_CLOSE80
        for (posLouver = L_MID; posLouver >= L_CLOSE; posLouver--) {
          louver.write(posLouver);
          delay(5);
        }
      }
      else {
        posLouver = L_CLOSE;   //keep L_CLOSE80
      }
    }

    else {  //not raining nor thundering (move to inference using nano ble sense)
      ...
      ei_printf("SELECTED: %s\n\n", result.classification[selected].label);
      // move servos based on the "selected"
      if (selected == 0) {    //COMFORTABLE, W_OPEN140, L_OPEN160
        //OPEN THE WINDOW SERVO
        if (posWindow <= W_CLOSE) {   // from W_CLOSE0 to W_OPEN140
          for (posWindow = W_CLOSE; posWindow <= W_OPEN; posWindow++) {
            window.write(posWindow);
            delay(5);
          }
        }
        else if (posWindow == W_MID || posWindow == W_MID+1 || posWindow == W_MID-1) {   // from W_MID60 to W_OPEN140
          for (posWindow = W_MID; posWindow <= W_OPEN; posWindow++) {
            window.write(posWindow);
            delay(5);
          }
        }
        else {
          posWindow = W_OPEN;   //keep W_OPEN140
        }
        //OPEN THE LOUVER SERVO
        if (posLouver <= L_CLOSE) {   // from L_CLOSE80 to L_OPEN160
          for (posLouver = L_CLOSE; posLouver <= L_OPEN; posLouver++) {
          ...
          posLouver = L_OPEN;   //keep L_OPEN160
        }
      }
      
      else if (selected == 1) {   //OVERCAST, W_OPEN140, L_MID120
        //OPEN THE WINDOW SERVO
        if (posWindow <= W_CLOSE) {   // from W_CLOSE0 to W_OPEN140
          for (posWindow = W_CLOSE; posWindow <= W_OPEN; posWindow++) {
            window.write(posWindow);
        ...
        // MID THE LOUVER SEFVO
        if (posLouver <= L_CLOSE) {   // from L_CLOSE80 to L_MID120
          for (posLouver = L_CLOSE; posLouver <= L_MID; posLouver++) {
            louver.write(posLouver);
            delay(5);
          }
        }
        else if (posLouver >= L_OPEN) {   // from L_OPEN160 to L_MID120
          for (posLouver = L_OPEN; posLouver >= L_MID; posLouver--) {
            louver.write(posLouver);
            delay(5);
          }
        }
        else {
          posLouver = L_MID;    //keep L_MID120
        }
      }

      else if (selected == 2) {   //SUNNY DRY, W_MID60, L_CLOSE80
        //MID THE WINDOW SERVO
        if (posWindow <= W_CLOSE) {   // from W_CLOSE0 to W_MID60
          for (posWindow = W_CLOSE; posWindow <= W_MID; posWindow++) {
            window.write(posWindow);
            delay(5);
          }
        }
        else if (posWindow >= W_OPEN) {   // from W_OPEN140 to W_MID60
          for (posWindow = W_OPEN; posWindow >= W_MID; posWindow--) {
            window.write(posWindow);
            delay(5);
          }
        }
        else {
          posWindow = W_MID;    //keep W_MID60
        }
        //CLOSE THE LOUVER SERVO
        if (posLouver >= L_OPEN) {   // from L_OPEN160 to L_CLOSE80
          for (posLouver = L_OPEN; posLouver >= L_CLOSE; posLouver--) {
      ...
      else {    //SUNNY HUMID, W_CLOSE0, L_CLOSE80
        //CLOSE THE WINDOW WERVO
        if (posWindow >= W_OPEN) {   // from W_OPEN140 to W_CLOSE0
          for (posWindow = W_OPEN; posWindow >= W_CLOSE; posWindow--) {
            window.write(posWindow);
...
void receiveEvent(int howMany) {
  ...
  if (x == 2) {
    thunderCount++;
    Serial.print("thunderCount: ");
    Serial.println(thunderCount);
  }
}
